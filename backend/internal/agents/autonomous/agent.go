...
type AutonomousTask struct {
	...
	PausedFrom  AgentState             `json:"paused_from,omitempty"`
...
func (a *AutonomousAgent) addLog(task *AutonomousTask, level LogLevel, message string, stepID string) {
	...
	taskLabel := task.ID
	if len(taskLabel) > 8 {
		taskLabel = taskLabel[:8]
	}
	log.Printf("[%s] Task %s: %s", level, taskLabel, message)
}
...
func (a *AutonomousAgent) ResumeTask(taskID string) error {
	...
	resumeTo := StateExecuting
	task.mu.Lock()
	if task.PausedFrom != "" && task.PausedFrom != StatePaused {
		resumeTo = task.PausedFrom
	}
	task.PausedFrom = ""
	task.mu.Unlock()

	a.setTaskState(task, resumeTo)
