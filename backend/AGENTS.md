...
- The `APEX_AI_TESTING_PROFILE=ollama-credit-saver` profile is the explicit exception for launch-cost-saving canaries: it may replace configured Claude/GPT/Gemini/Grok provider slots with Ollama-backed alias clients so normal provider-routing paths can be tested without spending flagship-provider credits. This is test evidence for routing/build behavior, not evidence that the real flagship provider accounts have credits or production health.
