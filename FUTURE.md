...
| FT-006 | Active | Billing Launch Readiness | Make Stripe subscription, credit purchase, and invoice handling safe enough for public signup. | Configure launch-matching Stripe price IDs and replay real Stripe test webhooks through the deployed endpoint after local replay coverage. | `/health/features` reports payments ready, local replay tests pass, Stripe CLI/dashboard replay proves duplicate delivery, and controlled live checkout plus billing portal flows succeed. |
