...
for (const existing of fs.readdirSync(outDir)) {
  if (existing.endsWith('.smoke.spec.ts')) {
...
for (const file of manifestFiles) {
  ...
  test('auth: ${escapeTs(flowName)}', async ({ request }) => {
    ...
    const clientHeaders = makeClientHeaders('auth-${idx + 1}-' + suffix);
  ...
  const previewProxyTests = previewProxyScenarios.map((scenario, idx) => {
      ...
      return `
    {
      const res${i} = await request.fetch(joinProxyURLPath(proxyBase, ${JSON.stringify(p)}), {
        ...
        headers: withAuthHeaders(clientHeaders, token)
      });
  ...
  test('preview-proxy: ${escapeTs(scenarioName)}', async ({ request, page }) => {
    const apiBase = process.env.PLAYWRIGHT_API_URL || 'http://localhost:8080';
    const suffix = \`\${Date.now()}-\${Math.floor(Math.random() * 100000)}\`;
    const clientHeaders = makeClientHeaders('preview-${idx + 1}-' + suffix);
  ...
  test('preflight: ${escapeTs(scenarioName)}', async ({ request }) => {
    const apiBase = process.env.PLAYWRIGHT_API_URL || 'http://localhost:8080';
    const suffix = \`\${Date.now()}-\${Math.floor(Math.random() * 100000)}\`;
    const clientHeaders = makeClientHeaders('preflight-${idx + 1}-' + suffix);
  ...
  test('build-error: ${escapeTs(scenarioName)}', async ({ request }) => {
    const apiBase = process.env.PLAYWRIGHT_API_URL || 'http://localhost:8080';
    const suffix = \`\${Date.now()}-\${Math.floor(Math.random() * 100000)}\`;
    const clientHeaders = makeClientHeaders('build-error-${idx + 1}-' + suffix);
  ...
  const helperBlock = (authFlows.length > 0 || preflightScenarios.length > 0 || buildErrorScenarios.length > 0 || previewProxyScenarios.length > 0) ? `
function makeClientHeaders(seed) {
  const hash = Array.from(String(seed)).reduce((acc, ch) => ((acc * 33) + ch.charCodeAt(0)) >>> 0, 5381);
  const octet3 = 1 + (hash % 250);
  const octet4 = 1 + ((hash >>> 8) % 250);
...
${helperBlock}${previewHelperBlock}test.describe('${escapeTs(manifest.name || id)} (${id})', () => {${healthTests}${routeTests}${uiTests}${authFlowTests}${preflightTests}${buildErrorTests}${previewProxyTests}
});
`;
