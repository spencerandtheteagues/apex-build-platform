...
` : '';
  ...
  async function getSharedAuth(request) {
    ...
    expect(loginRes.ok()).toBeTruthy();
    const loginBody = await loginRes.json();
    const token = loginBody?.data?.tokens?.access_token ?? loginBody?.tokens?.access_token;
    expect(typeof token).toBe('string');
    expect(token.length).toBeGreaterThan(20);

    sharedAuthState = { username, email, password, token };
    return sharedAuthState;
  }

` : '';
  const helperBlock = `${previewHelperBlock}${sharedAuthBlock}`;
  const projectGuardBlock = apiOnly
    ? `
  test.skip(({ browserName }) => browserName !== 'chromium', 'API-only smoke runs once in Chromium to avoid duplicate auth churn across browser projects');
`
    : '';
