...
for (const file of manifestFiles) {
  ...
  const sharedAuth = manifest.sharedAuth === true;
  const apiOnly =
    routes.length === 0 &&
    assertions.length === 0 &&
    previewProxyScenarios.length === 0;
  const sharedAuthFlow = authFlows[0] || {};
  const sharedAuthPrefix = String(sharedAuthFlow.usernamePrefix || `pw_${id}_`);
  const sharedAuthPassword = String(sharedAuthFlow.password || 'Password123!');
  const sharedAuthFullName = String(sharedAuthFlow.fullName || 'Playwright E2E');
  const needsAuthHelpers =
    authFlows.length > 0 ||
    previewProxyScenarios.length > 0 ||
    preflightScenarios.length > 0 ||
    buildErrorScenarios.length > 0 ||
    sharedAuth;
  ...
  test('auth: ${escapeTs(flowName)}', async ({ request }) => {
    ...
    expect(typeof auth.cookie).toBe('string');
    expect(auth.cookie.length).toBeGreaterThan(20);
  ...
  test('auth: ${escapeTs(flowName)}', async ({ request }) => {
    ...
    ${register ? `auth = await registerAuthSession(request, apiBase, username, email, password, 'Playwright E2E');` : ''}
    ${login && !register ? `auth = await loginAuthSession(request, apiBase, username, email, password);` : ''}
    ${login || register ? `expect(auth).not.toBeNull();
    expect(typeof auth.cookie).toBe('string');
    expect(auth.cookie.length).toBeGreaterThan(20);
    expect(typeof auth.token).toBe('string');
    expect(auth.token.length).toBeGreaterThan(20);` : ''}
  ...
  test('preview-proxy: ${escapeTs(scenarioName)}', async ({ request, page }) => {
    ...
    const auth = await registerAuthSession(request, apiBase, username, email, password, 'Playwright Preview');
  ...
  test('preflight: ${escapeTs(scenarioName)}', async ({ request }) => {
    ...
    const auth = await registerAuthSession(request, apiBase, username, email, 'Preflight123!', 'Playwright Preflight');
  ...
  test('build-error: ${escapeTs(scenarioName)}', async ({ request }) => {
    ...
    const auth = await registerAuthSession(request, apiBase, username, email, 'BuildErr123!', 'Playwright BuildErr');
  ...
  const authHelperBlock = needsAuthHelpers ? `
...
async function csrfToken(request, apiBase) {
  const res = await request.get(apiBase + '/api/v1/csrf-token');
...
async function authSessionFromResponse(request, apiBase, response, username, email, password) {
  const cookie = cookieHeaderFromResponse(response);
  expect(cookie.length).toBeGreaterThan(20);
  const token = cookieValue(cookie, 'apex_access_token');
  ...
  return {
    ...
    cookie,
    token,
    csrfToken: await csrfToken(request, apiBase),
  };
}

async function registerAuthSession(request, apiBase, username, email, password, fullName) {
  const registerRes = await request.post(apiBase + '/api/v1/auth/register', {
    data: { username, email, password, full_name: fullName, accept_legal_terms: true }
  });
  expect([200, 201]).toContain(registerRes.status());
  return await authSessionFromResponse(request, apiBase, registerRes, username, email, password);
}

async function loginAuthSession(request, apiBase, username, email, password) {
  const loginRes = await request.post(apiBase + '/api/v1/auth/login', {
    data: { username, email, password }
  });
  expect(loginRes.ok()).toBeTruthy();
  return await authSessionFromResponse(request, apiBase, loginRes, username, email, password);
}

` : '';
  const previewHelperBlock = previewProxyScenarios.length > 0 ? `
...
` : '';
  const sharedAuthBlock = sharedAuth ? `
  let sharedAuthState = null;

  async function getSharedAuth(request) {
    if (sharedAuthState) {
    ...
    const apiBase = process.env.PLAYWRIGHT_API_URL || 'http://localhost:8080';
    const suffix = \`\${Date.now()}-\${Math.floor(Math.random() * 100000)}\`;
    const username = ${JSON.stringify(sharedAuthPrefix)} + suffix;
    const email = username + '@example.com';
    const password = ${JSON.stringify(sharedAuthPassword)};

    sharedAuthState = await registerAuthSession(request, apiBase, username, email, password, ${JSON.stringify(sharedAuthFullName)});
    return sharedAuthState;
  }

` : '';
  const helperBlock = `${authHelperBlock}${previewHelperBlock}${sharedAuthBlock}`;
  const projectGuardBlock = apiOnly
    ? `
  test.beforeEach(({}, testInfo) => {
    test.skip(testInfo.project.name !== 'chromium', 'API-only smoke runs once in Chromium to avoid duplicate auth churn across browser projects');
  });
`
    : '';
...
${helperBlock}test.describe('${escapeTs(manifest.name || id)} (${id})', () => {${projectGuardBlock}${healthTests}${routeTests}${uiTests}${authFlowTests}${preflightTests}${buildErrorTests}${previewProxyTests}
});
`;
